...
def process_otp_number(message):
    ...
    wait = bot.send_message(message.chat.id, "⏳ OTP চেক করা হচ্ছে...")
    status, detail = check_otp_arrived(stored_link, number)
    try:
        bot.delete_message(message.chat.id, wait.message_id)
    except Exception:
    ...
    if status == "arrived":
        otp_text = detail if _OTP_CODE_RE.fullmatch(detail or "") else "পাওয়া গেছে, কিন্তু কোড আলাদাভাবে বোঝা যায়নি — লিংকে দেখুন"
        bot.send_message(
            message.chat.id,
            "✅ <b>OTP Found</b>\n\n"
            f"📞 <b>Number:</b> <code>{number}</code>\n"
            f"🔑 <b>OTP:</b> <code>{otp_text}</code>\n"
        ...
        bot.send_message(
            message.chat.id,
            f"❌ এই নম্বরে <code>{number}</code> এখনো কোনো OTP আসেনি।\n🕒 কিছুক্ষণ পর আবার চেক করুন।",
            reply_markup=main_menu_keyboard(uid),
        )
    else:
        bot.send_message(
            message.chat.id,
            "⚠️ এখন OTP চেক করা সম্ভব হয়নি। কিছুক্ষণ পর আবার চেষ্টা করুন।",
            reply_markup=main_menu_keyboard(uid),
        )
        _notify_admins(
            "⚠️ <b>OTP চেক ব্যর্থ (unknown)</b>\n"
            f"👤 User: <code>{uid}</code>\n📞 Number: <code>{number}</code>\n"
            f"🔗 Link: {stored_link}\n❓ কারণ: {detail}\n\n"
            "Admin Panel → 🔍 Debug OTP Link দিয়ে এই লিংকের raw রেসপন্স দেখুন।"
        )
