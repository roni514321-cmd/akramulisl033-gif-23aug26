...
def _normalize_database_url(url: str) -> tuple[str, bool]:
    ...
    ✅ Aiven-এর MySQL URL এ প্রায়ই '?ssl-mode=REQUIRED' জুড়ে থাকে।
    SQLAlchemy URL-এর query param গুলো সরাসরি ড্রাইভারের connect()-এ
    kwarg হিসেবে পাঠায়, কিন্তু asyncmy 'ssl-mode' নামে kwarg চেনে না —
    ফলে এরর হয়: "connect() got an unexpected keyword argument 'ssl-mode'"।
    তাই এই param URL থেকে বাদ দিয়ে, SSL দরকার কিনা আলাদাভাবে বুঝে
    ফেরত দেওয়া হচ্ছে — SSL context connect_args দিয়ে বসানো হবে।"""
    u = url.strip()
    if u.startswith("mysql://"):
        u = "mysql+asyncmy://" + u[len("mysql://"):]
    elif u.startswith("mysql+pymysql://"):
        u = "mysql+asyncmy://" + u[len("mysql+pymysql://"):]
    ...
    if "?" in u:
        ...
        for part in query.split("&"):
            ...
            if key.lower() in ("ssl-mode", "ssl_mode"):
                if val.strip().upper() in ("REQUIRED", "VERIFY_CA", "VERIFY_IDENTITY", "1", "TRUE"):
                    ssl_required = True
                continue
            kept_params.append(part)
        u = base + ("?" + "&".join(kept_params) if kept_params else "")

    return u, ssl_required


DATABASE_URL, _DB_SSL_REQUIRED = _normalize_database_url(
...
def _build_connect_args(database_url: str, ssl_required: bool) -> dict:
    """asyncmy/asyncpg কে সঠিকভাবে SSL enable করে দেয়, যখন Aiven-এর
    URL এ ssl-mode=REQUIRED ছিল (এখন সেটা URL থেকে বাদ দেওয়া হয়েছে)।"""
    if not ssl_required:
        return {}
    if database_url.startswith("mysql+asyncmy://"):
        import ssl as _ssl
        ctx = _ssl.create_default_context()
        # ✅ Aiven 'ssl-mode=REQUIRED' মানে শুধু encryption দরকার, CA verify না।
        # Aiven নিজস্ব (self-signed) CA ব্যবহার করে, যা পাবলিক CA store-এ নেই —
        # তাই default context দিয়ে verify করলে CERTIFICATE_VERIFY_FAILED হয়।
...
engine = create_async_engine(
    DATABASE_URL,
    echo=False,
    connect_args=_build_connect_args(DATABASE_URL, _DB_SSL_REQUIRED),
    # ✅ Pool tuning — এগুলো ছাড়া SQLAlchemy default (pool_size=5) দিয়ে চলছিল,
    # যেটা ২০০০ ইউজারের লোডে বারবার নতুন কানেকশন খুলতে বাধ্য করে (প্রতিটাতে
    # Aiven পর্যন্ত TLS handshake লাগে — এটাই মূলত স্লো রেসপন্সের বড় কারণ)।
    pool_size=8,
    max_overflow=7,          # Aiven-এর ছোট প্ল্যানে connection limit কম থাকে (সাধারণত ~20), তাই এর বেশি না নেওয়াই নিরাপদ
    pool_recycle=280,       # Aiven idle কানেকশন কেটে দেওয়ার আগেই recycle করে
    pool_pre_ping=True,     # মরা কানেকশন দিয়ে কুয়েরি চালিয়ে টাইমআউটে আটকানো ঠেকায়
)
...
_maintenance_cache = {"value": False, "ts": 0.0}
_MAINTENANCE_CACHE_TTL = 15  # সেকেন্ড — প্রতিটা মেসেজ/ক্লিকে DB কল এড়িয়ে fast response দেয়
...
async def is_maintenance_on(session: AsyncSession) -> bool:
    ...
    val = (await get_setting(session, "maintenance_mode", "0")) == "1"
...
# =====================================================================
_product_list_cache = {}       # {page: (text, markup, ts)}
_PRODUCT_LIST_CACHE_TTL = 10    # সেকেন্ড — মার্কেট মেনু বারবার খোলা হয়, তাই ছোট TTL cache দিয়ে বাকি বটের মতো fast করা
...
async def render_product_list(page: int = 0) -> tuple[str, Optional[InlineKeyboardMarkup]]:
    ...
    cached = _product_list_cache.get(page)
    if cached and (time.time() - cached[2]) < _PRODUCT_LIST_CACHE_TTL:
    ...
    async with SessionLocal() as s:
        ...
        if not all_products:
            result = ("🛒 <b>Marketplace</b>\n\nNo products available yet.", None)
            _product_list_cache[page] = (result[0], result[1], time.time())
            return result
        ...
        # ✅ আগে প্রতিটা প্রোডাক্টের জন্য আলাদা COUNT() query চলত (N+1 problem —
        # পেজে ৫টা প্রোডাক্ট থাকলে ৫টা আলাদা Aiven round-trip লাগত)। এখন একটাই
        # grouped query দিয়ে সব stock count একসাথে আনা হচ্ছে।
        chunk_ids = [p.id for p in chunk]
        stock_counts = {pid: 0 for pid in chunk_ids}
        if chunk_ids:
            rows = (await s.execute(
                select(InventoryItem.product_id, func.count())
                .where(InventoryItem.product_id.in_(chunk_ids), InventoryItem.status == "available")
    ...
    _product_list_cache[page] = (text, markup, time.time())
    return text, markup
...
async def msg_deposit_trxid(message: Message, state: FSMContext, db_user: User):
    ...
    id_label = "Pay Order ID" if method == "Binance" else "TrxID"
    trx_id = (message.text or "").strip().upper()
    trx_id = re.sub(r"\s+", "", trx_id)

    if len(trx_id) < 4 or not re.match(r"^[A-Z0-9]+$", trx_id):
        source_hint = "Binance-এর Pay Order ID" if method == "Binance" else "SMS থেকে"
    ...
    async with SessionLocal() as s:
        # একই TrxID/Pay Order ID দিয়ে আগে কোনো approved ডিপোজিট থাকলে ডুপ্লিকেট আটকানো হচ্ছে
        dup = (await s.execute(
            select(Deposit).where(
    ...
    wait_note = (
        ...
        "Payment received হলে automatic approved হয়ে যাবে"
    )
...
async def cb_toggle_maintenance(call: CallbackQuery):
    ...
    _maintenance_cache["ts"] = 0.0  # cache invalidate — সাথে সাথে নতুন state সবাই দেখবে
    await call.answer("Updated!")
