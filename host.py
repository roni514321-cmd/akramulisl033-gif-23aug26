    ...
    async with SessionLocal() as s:
        ...
        if dep:
            ...
            if amount_ok and method_ok:
                ...
                # ❌ TrxID মিলেছে কিন্তু amount/method মিলেনি — auto-approve না
                # করে admin কে সুনির্দিষ্ট mismatch error পাঠানো হচ্ছে।
                mismatch_lines = []
                if not method_ok:
                ...
                try:
                    await broadcast_to_admins(
                        "⚠️ <b>Deposit Auto-Approve Failed — Mismatch</b>\n\n"
                        f"🆔 Deposit: DEP-{dep.id} (user {dep.user_id})\n"
                        f"🔑 TrxID: <code>{sms.trx_id}</code> (মিলেছে)\n\n"
                        + "\n".join(mismatch_lines) +
                        "\n\n👉 ম্যানুয়ালি রিভিউ করে Approve/Reject করুন।"
                    )
                except Exception:
                    pass
...
async def try_auto_approve_from_stored_sms(session: AsyncSession, dep: Deposit) -> bool:
    ...
    সাথে সাথে অটো-অ্যাপ্রুভ করে। TrxID মিললেও method/amount না মিললে admin কে
    mismatch alert পাঠিয়ে pending রেখে দেয়।"""
    if not dep.trx_id:
        return False
    ...
    # ❌ TrxID মিলেছে কিন্তু amount/method মিলেনি — error alert পাঠাও, approve না করে
    mismatch_lines = []
    if not method_ok:
        mismatch_lines.append(f"💳 Method মিলছে না — Deposit: <b>{dep.method or '—'}</b> vs SMS: <b>{sms.method or '—'}</b>")
    if not amount_ok:
...
def kb_products(products: list[Product], page: int, has_next: bool, stock_counts: Optional[dict] = None) -> InlineKeyboardMarkup:
    ...
    b = InlineKeyboardBuilder()
    for p in products:
        # ✅ FIX: বাটনের লেবেলে আর price/stock repeat করা হচ্ছে না — এই তথ্য
        # উপরের মেসেজ টেক্সটেই (render_product_list) দেখানো হয়, বাটনে শুধু
        # নামটাই যথেষ্ট, দুইবার একই লাইন না দেখানোর জন্য।
        b.button(text=f"🛒 {p.name}", callback_data=ProdCB(id=p.id))
    b.adjust(1)
    nav_row = []
    if page > 0:
