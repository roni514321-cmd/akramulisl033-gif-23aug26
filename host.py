...
# KEYBOARDS
# =====================================================================
def kb_home(user_is_admin: bool = False) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.button(text="🛒 Marketplace", callback_data=NavCB(to="market"))
    b.button(text="👤 Profile", callback_data=NavCB(to="profile"))
    b.button(text="💰 Wallet", callback_data=NavCB(to="wallet"))
    b.button(text="📦 My Orders", callback_data=NavCB(to="orders"))
    b.button(text="💼 Sell", callback_data=NavCB(to="sell"))
    b.button(text="📞 Support", callback_data=NavCB(to="support"))
    b.adjust(2, 2, 2)
    if user_is_admin:
        b.row(InlineKeyboardButton(text="🛠 Admin Panel", callback_data=NavCB(to="admin").pack()))
    return b.as_markup()
...
async def cmd_start(message: Message, db_user: User, state: FSMContext):
    await state.clear()
    await message.answer(home_text(db_user), reply_markup=kb_home(is_admin(message.from_user.id)))
...
async def cb_home(call: CallbackQuery, db_user: User, state: FSMContext):
    ...
    await safe_edit(call, home_text(fresh), kb_home(is_admin(call.from_user.id)))
    await call.answer()
