...
class Deposit(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(BigInteger, nullable=False)
    amount = Column(Float, nullable=False)  # সবসময় BDT — Binance-এর ক্ষেত্রে rate দিয়ে কনভার্ট করা
    method = Column(String(32), nullable=False)
    trx_id = Column(String(64), nullable=True, index=True)  # ইউজারের দেওয়া TrxID/Binance username — SMS এর সাথে ম্যাচ করাতে ব্যবহার হয়
    usdt_amount = Column(Float, nullable=True)  # শুধু Binance-এর জন্য — ইউজার যেই USDT amount টা টাইপ করেছে (SMS ম্যাচিং এটার সাথেই হয়, BDT amount-এর সাথে না)
    note = Column(String(256), nullable=True, default="")
...
class SmsMessage(Base):
    ...
    থাকে। ইউজারের জমা দেওয়া amount + trx_id (Binance-এ trx_id = sender
    username/নেইম) এর সাথে ম্যাচ করে অটো-অ্যাপ্রুভের জন্য ব্যবহার হয়।"""
...
async def init_db():
    async with engine.begin() as conn:
        ...
        try:
            await conn.exec_driver_sql(
                "ALTER TABLE deposits ADD COLUMN usdt_amount FLOAT"
            )
        except Exception:
            pass
...
# 🪙 Binance ওয়ালেট নোটিফিকেশন — এতে কোনো TrxID থাকে না, বরং যে (ইউজার)নেইম
# থেকে টাকা এসেছে সেটাই থাকে — এই নেইমে স্পেসও থাকতে পারে (যেমন "Garth
# Mantifel wpgu")। তাই trx_id ফিল্ডে (normalize করে, uppercase + single-space)
# এই নেইমটা সেইভ করা হয় — matching logic-এর বাকি সবকিছু অপরিবর্তিত থাকে।
# উদাহরণ: "You have received a payment of 0.1 USDT from Garth Mantifel wpgu on 2026-09-15 04:01:53(UTC)"
...
def _normalize_binance_name(name: str) -> str:
    """Binance sender নেইম normalize করে — শুধু বাড়তি স্পেস একটায় নামিয়ে আনা হয়,
    small/capital letter যেভাবে আসে ঠিক সেভাবেই রাখা হয় (case পরিবর্তন করা হয় না)।"""
...
def parse_payment_sms(text: str, hint_method: str = "", sender: str = "") -> Optional[dict]:
    ...
    # ফরম্যাটে TrxID থাকে না, বরং sender নেইম (স্পেসসহ হতে পারে) থাকে, তাই
    # নিচের bKash-স্টাইল extraction চালানোর আগেই এটা আলাদাভাবে হ্যান্ডেল করা হয়।
...
def _methods_match(a: str, b: str) -> bool:
    """Payment method দুটো (bKash/Nagad/Rocket/Binance) case-insensitively মিলছে কিনা।
...
def _dep_compare_amount(dep: "Deposit") -> Optional[float]:
    """Deposit-এর সাথে SMS/নোটিফিকেশনের amount কোনটার সাথে মেলাতে হবে সেটা
    ঠিক করে। Binance-এ ইউজার USDT amount টাইপ করে আর SMS/নোটিফিকেশনেও USDT
    amount-ই থাকে — তাই সেই usdt_amount-টার সাথেই মেলাতে হবে, BDT-তে কনভার্ট
    হওয়া dep.amount-এর সাথে না (কনভার্ট করা amount কখনোই USDT figure-এর সাথে
    মিলবে না)।"""
    if dep.method == "Binance":
        return dep.usdt_amount if dep.usdt_amount is not None else None
    return dep.amount


def _amounts_match(method: str, a: Optional[float], b: Optional[float]) -> bool:
    if a is None or b is None:
        return False
    eps = 0.0005 if method == "Binance" else 0.01  # USDT-তে খুবই ছোট amount (0.1 ইত্যাদি) হয়, তাই টাইট টলারেন্স
    return abs(a - b) < eps
...
async def store_sms_and_try_match(text: str, sender: str = "", hint_method: str = "") -> dict:
    ...
    ডিপোজিটের সাথে ম্যাচ করে থাকলে সাথে সাথে অটো-অ্যাপ্রুভ করে দেয়। রেজাল্ট dict
    রিটার্ন করে। ফরোয়ার্ডার অ্যাপ রিট্রাই করলে (একই SMS দুইবার আসলে) ডুপ্লিকেট
    রো তৈরি করে না।
    ✅ FIX: শুধু TrxID মিললেই আর অটো-অ্যাপ্রুভ হবে না — payment method, amount ও
    TrxID/Username — এই ৩টাই মিললে তবেই approve হবে। TrxID মিললেও method/amount
    না মিললে admin কে error/mismatch alert যাবে, deposit pending-ই থাকবে।"""
    parsed = parse_payment_sms(text, hint_method, sender=sender)
    if not parsed:
        return {"stored": False, "reason": "not_a_payment_sms"}

    async with SessionLocal() as s:
        # ⚠️ Binance-এ trx_id আসলে sender username/নেইম — ইউনিক transaction
        # identifier না (একই ইউজার বারবার একই amount বৈধভাবে পাঠাতে পারে)।
        # তাই সেখানে raw SMS টেক্সট হুবহু মিললে তবেই সেটাকে "app এর retry"
        # ধরা হয়। bKash/Nagad/Rocket-এ TrxID সত্যিকারের ইউনিক বলে trx_id+amount
        # দিয়ে চেক করাই যথেষ্ট।
        ...
        dep = (await s.execute(
            select(Deposit).where(
                ...
                func.upper(Deposit.trx_id) == sms.trx_id.upper(),
            ).order_by(Deposit.id.asc())
        )).scalars().first()

        matched_dep_id = None
        if dep:
            amount_ok = _amounts_match(sms.method, _dep_compare_amount(dep), sms.amount)
            ...
            if amount_ok and method_ok:
                ...
                if not amount_ok:
                    mismatch_lines.append(f"💰 Amount মিলছে না — Deposit: <b>{_dep_compare_amount(dep)} {_amount_unit(dep.method)}</b> vs SMS: <b>{sms.amount} {_amount_unit(sms.method)}</b>")
...
async def try_auto_approve_from_stored_sms(session: AsyncSession, dep: Deposit) -> str:
    ...
    sms = (await session.execute(
        select(SmsMessage).where(
            ...
            func.upper(SmsMessage.trx_id) == dep.trx_id.strip().upper(),
        ).order_by(SmsMessage.id.asc())
    )).scalars().first()
    if not sms:
        return "no_match"

    amount_ok = _amounts_match(dep.method, _dep_compare_amount(dep), sms.amount)
    method_ok = _methods_match(dep.method, sms.method)
    if amount_ok and method_ok:
...
def _normalize_video_url(url: str) -> str:
    """method_video_url সেটিং-এ admin স্কিমা (https://) ছাড়া লিংক দিলেও যেন
    Telegram inline বাটনে valid URL হিসেবে কাজ করে, তাই এখানে যোগ করে দেওয়া হয়।"""
    u = (url or "").strip()
    if not u:
        return u
...
@router.message(F.text == "🎬 Method")
async def msg_method(message: Message, state: FSMContext):
    await state.clear()
    async with SessionLocal() as s:
        video_url = await get_setting(s, "method_video_url", "")
    if not video_url:
        await message.answer(
            "🎬 <b>Payment Method Tutorial</b>\n\n"
            "⚠️ এখনো কোনো ভিডিও লিংক সেট করা হয়নি। সাপোর্টে যোগাযোগ করুন।",
        )
        return
    b = InlineKeyboardBuilder()
    b.button(text="▶️ Watch Video", url=_normalize_video_url(video_url))
    b.adjust(1)
    await message.answer(
        "🎬 <b>Payment Method Tutorial</b>\n\n"
        "👇 কীভাবে পেমেন্ট করবেন তা এই ভিডিওতে দেখানো হয়েছে:",
...
async def _edit_or_send_raw(
    message: Message, prompt_id: Optional[int], text: str, markup: Optional[InlineKeyboardMarkup] = None
) -> Optional[int]:
    """edit_flow_message-এর মূল লজিক, কিন্তু state ছাড়াই — যেই জায়গায় flow
    শেষ হওয়ার আগে state.clear() করা হয়ে গেছে (যেমন deposit submit হওয়ার
    পর), সেখানে ধরে রাখা prompt_id দিয়ে সরাসরি একই মেসেজ এডিট করার জন্য।
    ইউজারের টাইপ করা মেসেজ (amount/TrxID ইত্যাদি) ডিলিট করা হয় না — সেটা
    রেকর্ডের জন্য চ্যাটে থেকে যায়, শুধু বটের প্রম্পটটাই এডিট হয়।"""
    new_id = prompt_id
    if prompt_id:
        try:
            await bot.edit_message_text(text, chat_id=message.chat.id, message_id=prompt_id, reply_markup=markup)
        except TelegramBadRequest:
            try:
...
async def edit_flow_message(
    message: Message, state: FSMContext, key: str, text: str, markup: Optional[InlineKeyboardMarkup] = None
) -> None:
    """মাল্টি-স্টেপ ফ্লো (যেমন Deposit) এ ইউজার টেক্সট টাইপ করে পরের ধাপে গেলে,
    নতুন আলাদা মেসেজ না পাঠিয়ে আগের বট-মেসেজটাই এডিট করে পরের অপশন/প্রম্পট
    দেখানো হয়। `key` দিয়ে state-এ সেভ করা প্রম্পট মেসেজ-আইডি খোঁজা হয়; এডিট
    করার পর নতুন মেসেজ-আইডি আবার state-এ সেভ করা হয় (যাতে fallback দিয়ে নতুন
    মেসেজ পাঠানো লাগলেও পরের ধাপে সেটাই এডিট হয়)। ইউজারের টাইপ করা
    amount/TrxID মেসেজ ডিলিট করা হয় না — শুধু বটের প্রম্পটটাই এডিট হয়।"""
    data = await state.get_data()
    prompt_id = data.get(key)
    new_id = await _edit_or_send_raw(message, prompt_id, text, markup)
    if new_id != prompt_id:
        await state.update_data(**{key: new_id})
...
async def cb_deposit_method(call: CallbackQuery, callback_data: DepMethodCB, state: FSMContext):
    ...
    await state.update_data(dep_prompt_id=call.message.message_id)
    await call.answer()
...
async def msg_deposit_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        await edit_flow_message(
            message, state, "dep_prompt_id",
            f"❌ Invalid {bad_label}. Please send a valid positive number.",
        )
        return
    ...
    if min_dep > 0 and amount < min_dep:
        await edit_flow_message(
            message, state, "dep_prompt_id",
            f"❌ Minimum deposit amount is {dual_money(min_dep)}. Please enter a higher amount.",
        )
        return
    ...
    if method == "Binance":
        await edit_flow_message(
            ...
            "যে Binance username/নেইম থেকে টাকা পাঠিয়েছেন সেটা হুবহু লিখে পাঠান:👇",
        )
    else:
        await edit_flow_message(message, state, "dep_prompt_id", "Send your transection Id(TrxID):👇")
...
async def msg_deposit_trxid(message: Message, state: FSMContext, db_user: User):
    ...
    prompt_id = data.get("dep_prompt_id")
    id_label = _id_label(method)
    raw_input = (message.text or "").strip()

    if method == "Binance":
        # Binance sender নেইমে স্পেস থাকতে পারে (যেমন "Garth Mantifel wpgu")
        # আর ছোট/বড় হাতের অক্ষর যেভাবে লিখবে সেভাবেই সেইভ হবে (case বদলানো
        # হয় না) — শুধু বাড়তি স্পেস normalize করা হয়, ম্যাচিং case-insensitive।
        trx_id = _normalize_binance_name(raw_input)
        valid = len(trx_id) >= 3 and re.match(r"^[A-Za-z0-9_. ]+$", trx_id)
    else:
        trx_id = re.sub(r"\s+", "", raw_input.upper())
        valid = len(trx_id) >= 4 and re.match(r"^[A-Z0-9]+$", trx_id)

    if not valid:
        source_hint = "যে Binance ইউজারনেম/নেইম থেকে টাকা পাঠিয়েছেন সেটা" if method == "Binance" else "SMS থেকে"
        await edit_flow_message(
            message, state, "dep_prompt_id",
            f"❌ এটা সঠিক {id_label} মনে হচ্ছে না। {source_hint} হুবহু কপি করে আবার পাঠান।",
        )
        return
    ...
    async with SessionLocal() as s:
        # ⚠️ Binance-এ trx_id আসলে username/নেইম — একই নেইম দিয়ে বারবার
        ...
        dep = Deposit(
            user_id=db_user.id, amount=amount, method=method, trx_id=trx_id, status="pending",
            usdt_amount=usdt_amount if method == "Binance" else None,
        )
        s.add(dep)
        await s.commit()
        await s.refresh(dep)
    ...
    if auto_result == "approved":
        ...
        # এখানে শুধু "Send TrxID" প্রম্পট মেসেজটা মুছে চ্যাট পরিষ্কার রাখা হচ্ছে,
        # যাতে পুরনো প্রম্পট ঝুলে না থাকে। ইউজারের দেওয়া TrxID মেসেজ অক্ষত থাকে।
        if prompt_id:
            try:
    ...
    if auto_result == "rejected_mismatch":
        ...
        # আর "pending, wait for approval" মেসেজ পাঠানো হবে না। পুরনো প্রম্পট মুছে
        # ফেলা হচ্ছে যাতে ঝুলে না থাকে। ইউজারের দেওয়া TrxID মেসেজ অক্ষত থাকে।
        if prompt_id:
            try:
                await bot.delete_message(message.chat.id, prompt_id)
            except Exception:
...
async def cb_admin_settings(call: CallbackQuery):
    ...
    async with SessionLocal() as s:
        ...
        method_video = await get_setting(s, "method_video_url", "")
    text = (
        "⚙️ <b>Settings</b>\n\n"
        f"🔧 Maintenance Mode: {'ON 🔴' if maint else 'OFF 🟢'}\n"
        ...
        f"🎬 Method Video: {method_video if method_video else '⚠️ Not set'}"
    )
    b = InlineKeyboardBuilder()
    ...
    b.button(text="🎬 Set Method Video", callback_data=NavCB(to="set_method_video"))
...
@router.callback_query(NavCB.filter(F.to.in_({"set_threshold", "set_methods", "set_support", "set_min_deposit", "set_min_withdraw", "set_binance_rate", "set_method_video"})))
async def cb_settings_edit_start(call: CallbackQuery, callback_data: NavCB, state: FSMContext):
    ...
    key_map = {
        ...
        "set_method_video": ("method_video_url", "🎬 Payment method ব্যাখ্যা করার ভিডিওর লিংক দিন (YouTube/Telegram/অন্য যেকোনো লিংক):\n\n(পরিষ্কার করতে <code>-</code> পাঠান)"),
    }
...
async def msg_settings_value(message: Message, state: FSMContext):
    ...
    elif key == "method_video_url" and value == "-":
        value = ""
    await state.clear()
    async with SessionLocal() as s:
