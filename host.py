...
def main_menu_keyboard(user_id):
    ...
    kb.add(types.KeyboardButton("💸 Refund"), types.KeyboardButton("🔑 OTP"))
    if is_admin(user_id):
...
def admin_panel_inline():
    ...
    kb.add(types.InlineKeyboardButton("🔍 Debug OTP Link", callback_data="admin_debug_link"))
...
def purchase_success_text(order):
    return (
        ...
        "🔑 OTP পেতে মেইন মেনু থেকে <b>OTP</b> বাটনে ক্লিক করে এই নম্বরটি দিন।"
    )
...
def cb_bulk_file_format(call):
    ...
    if fmt == "inline":
        ...
        header = (
            ...
            "🔑 প্রতিটি নম্বরের OTP পেতে মেইন মেনু থেকে <b>OTP</b> বাটনে ক্লিক করে নম্বরটি দিন।\n\n"
        )
        text = header
...
def cb_admin_panel(call):
    ...
    if data == "admin_debug_link":
        user_state[uid] = {"menu": "admin_debug_link"}
        bot.send_message(
            chat_id,
            "🔍 <b>Debug OTP Link</b>\n\nযে লিংকটা বট আসলে কী পাচ্ছে সেটা দেখতে চান, সেই OTP link পাঠান:",
        )
        bot.register_next_step_handler_by_chat_id(chat_id, process_admin_debug_link)
...
def _analyze_otp_text(raw_text, content_type="", number=""):
    ...
    is_html = (not is_json) and bool(re.search(r"<html|<body|<!doctype", raw_text or "", re.I))
    ...
    has_marker = any(h in low_early for h in _YES_OTP_HINTS)
    m = _OTP_CODE_RE.search(text)

    if is_html:
        # HTML পেজে (যেমন এই bot এর SMS panel) ভুল-শনাক্তি ঠেকাতে "Code Received"
        # মার্কার ছাড়া শুধু কোনো ৪-৮ ডিজিটের সংখ্যা পেলেই তা OTP ধরা হবে না
        # (ব্লক/এরর পেজে থাকা এলোমেলো সংখ্যা যেন ভুলে OTP ভাবা না হয়)
        if has_marker:
            return "arrived", (m.group(0) if m else "code-received marker found (code text unparsed)")
        if m:
            return "unknown", f"digits found ({m.group(0)}) but no 'Code Received' marker — সম্ভবত ভুল/ব্লক পেজ"
    else:
        if m:
...
# ---------------------------------------------------------------------------
# 🔑 OTP CHECK — ইউজার তার কেনা নম্বর দিলে বট লিংক চেক করে OTP দেখায়
# ---------------------------------------------------------------------------
@bot.message_handler(func=lambda m: m.text == "🔑 OTP")
def menu_otp(message):
    if maintenance_block(message):
        return
    if require_force_join(message):
        return
    uid = message.from_user.id
    user_state[uid] = {"menu": "otp_check"}
    bot.send_message(
        message.chat.id,
        "🔑 <b>OTP Check</b>\n\nযে নম্বরের OTP দেখতে চান, সেই নম্বরটি দিন:",
        reply_markup=back_to_main_keyboard(),
    )
    bot.register_next_step_handler(message, process_otp_number)


def _otp_link_keyboard(link):
    """OTP link এর জন্য বাটন বানায়। RAILWAY_URL https হলে আমাদের নিজের proxy
    দিয়ে https URL বানিয়ে আসল Telegram Mini App (WebApp) বাটন হিসেবে খোলে —
    Telegram শুধু https URL কেই WebApp বানায়। proxy সম্ভব না হলে (RAILWAY_URL
    নেই বা http) সাধারণ URL বাটনে fallback করে।"""
    proxied = _proxied_otp_url(link)
    kb = types.InlineKeyboardMarkup()
    if str(proxied).startswith("https://"):
        try:
            kb.add(types.InlineKeyboardButton("🔑 OTP দেখুন (Mini App)", web_app=types.WebAppInfo(url=proxied)))
            return kb
        except Exception:
            pass
    kb.add(types.InlineKeyboardButton("🔑 OTP দেখুন", url=link))
    return kb


def process_otp_number(message):
    uid = message.from_user.id
    text = (message.text or "").strip()
    if text in ["⬅️ Back", "⬅️ Back to Menu"]:
        go_back(message)
    ...
    number = item["number"]
    stored_link = item.get("otp_link")

    if not stored_link or not str(stored_link).startswith(("http://", "https://")):
        bot.send_message(
            message.chat.id,
            "⚠️ এই নম্বরের জন্য কোনো OTP লিংক পাওয়া যায়নি। এডমিনের সাথে যোগাযোগ করুন।",
            reply_markup=main_menu_keyboard(uid),
        )
        user_state[uid] = {"menu": "main"}
        return
    ...
    bot.send_message(
        ...
        "নিচের বাটনে ক্লিক করে OTP দেখুন 👇",
        reply_markup=_otp_link_keyboard(stored_link),
    )
    user_state[uid] = {"menu": "main"}
...
# ---------------------------------------------------------------------------
# 🔍 ADMIN DEBUG — বট আসলে লিংক থেকে কী রেসপন্স পায় সেটা raw দেখানোর জন্য
# ---------------------------------------------------------------------------
def process_admin_debug_link(message):
    chat_id = message.chat.id
    link = (message.text or "").strip()
    if not link.startswith(("http://", "https://")):
        bot.send_message(chat_id, "❌ এটা একটা valid লিংক না। http:// অথবা https:// দিয়ে শুরু করুন।")
        return
    ...
    try:
        ...
        bot.send_message(chat_id, f"❌ <b>Request Failed</b>\n\nবট এই লিংকে ঢুকতেই পারছে না।\nError: <code>{_html.escape(str(e))}</code>")
        return

    body = r.text or ""
    ct = r.headers.get("Content-Type", "N/A")
    snippet = _html.escape(body[:1500])
    status_result, detail = _analyze_otp_text(body, ct, "")

    bot.send_message(
        chat_id,
        f"📡 <b>HTTP Status:</b> <code>{r.status_code}</code>\n"
        f"📄 <b>Content-Type:</b> <code>{_html.escape(ct)}</code>\n"
        f"📏 <b>Body Length:</b> <code>{len(body)}</code> chars\n"
        f"🔎 <b>Bot এর সিদ্ধান্ত:</b> <code>{status_result}</code> ({_html.escape(str(detail))})\n\n"
        f"<b>Raw Response (প্রথম 1500 char):</b>\n<pre>{snippet}</pre>",
    )
...
if flask_app is not None:
    ...
    # -----------------------------------------------------------------
    # 🔁 OTP PANEL PROXY (http -> https mirror)
    # Telegram Mini App (WebApp) বাটন শুধু https URL এ কাজ করে, কিন্তু OTP
    # প্যানেলটা http/IP-based। তাই এই রুটটা আমাদের নিজের (https) Railway
    # ডোমেইনের মাধ্যমে ওই প্যানেলটাকে "আয়নার" মতো ফরওয়ার্ড করে দেখায় —
    # পেজের ভিতরের JS ও তার absolute-path API কলগুলো (/api/sms/...) এই
    # একই ডোমেইনে আসবে, এবং এই রুটটাই সেগুলো upstream এ ফরওয়ার্ড করে দেয়।
    # -----------------------------------------------------------------
    OTP_PANEL_ORIGIN = os.environ.get("OTP_PANEL_ORIGIN", "http://169.58.215.134:11111").rstrip("/")
    _PROXY_RESERVED_PATHS = {SMS_WEBHOOK_PATH.strip("/"), f"webhook/{BOT_TOKEN}"}
    _PROXY_HOP_HEADERS = {
    ...
    @flask_app.route("/<path:sub_path>", methods=["GET", "POST", "HEAD", "OPTIONS"])
    def otp_panel_proxy(sub_path):
        if sub_path.strip("/") in _PROXY_RESERVED_PATHS:
            from flask import abort
            abort(404)

        upstream_url = f"{OTP_PANEL_ORIGIN}/{sub_path}"
        ...
        try:
            upstream_resp = requests.request(
                ...
                url=upstream_url,
                headers={k: v for k, v in flask_request.headers.items() if k.lower() not in _PROXY_HOP_HEADERS},
        ...
        out_headers = [
            (k, v) for k, v in upstream_resp.headers.items() if k.lower() not in _PROXY_HOP_HEADERS
        ]
        return Response(upstream_resp.content, status=upstream_resp.status_code, headers=out_headers)


def _proxied_otp_url(link):
    """RAILWAY_URL (https) সেট থাকলে, লিংকটার path নিয়ে আমাদের নিজের https
    ডোমেইনের সাথে জুড়ে দেয় (proxy করে) — যাতে Telegram Mini App বাটনে ব্যবহার
    করা যায়। RAILWAY_URL না থাকলে (লোকাল polling টেস্ট) আসল লিংকটাই ফেরত দেয়।"""
    ...
    try:
        ...
        return RAILWAY_URL.rstrip("/") + path_and_query
    except Exception:
        return link
