...
def kb_products(products: list[Product], page: int, has_next: bool) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    for p in products:
        b.button(text=f"{p.name} — {dual_money(p.price)}", callback_data=ProdCB(id=p.id))
    b.adjust(1)
    nav_row = []
    if page > 0:
...
async def render_product_list(page: int = 0) -> tuple[str, Optional[InlineKeyboardMarkup]]:
    ...
    text = "\n".join(lines)
    markup = kb_products(chunk, page, has_next)
    return text, markup
...
async def cb_withdraw(call: CallbackQuery):
    ...
    if not methods:
        await call.answer("⛔ No withdrawal methods configured. Contact support.", show_alert=True)
        return
    text = (
        f"💸 <b>Withdraw</b>\n\n"
        f"💳 Available Balance: <b>{dual_money(user.balance)}</b>\n\n"
        f"একটি পেমেন্ট মেথড সিলেক্ট করুন:"
    )
    await safe_edit(call, text, kb_withdraw_methods(methods))
    await call.answer()
...
async def cb_withdraw_method(call: CallbackQuery, callback_data: WithdrawMethodCB, state: FSMContext):
    ...
    await safe_edit(
        ...
        f"💳 Available Balance: <b>{dual_money(user.balance)}</b>\n\n"
        f"কত টাকা তুলতে চান তা লিখুন (নাম্বার শুধু):",
        None,
    )
    await call.answer()
...
async def msg_withdraw_amount(message: Message, state: FSMContext, db_user: User):
    ...
    async with get_lock(f"user:{db_user.id}"):
        async with SessionLocal() as s:
            ...
            wd = Withdrawal(user_id=db_user.id, amount=amount, method=method, status="pending")
            s.add(wd)
            await s.commit()
            await s.refresh(wd)
