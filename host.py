...
def product_detail_keyboard():
    """3 keyboard buttons shown after selecting product: Buy one pcs, Bulk Buy, Back"""
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    kb.add(
        types.KeyboardButton("🛍️ Buy one pcs"),
        types.KeyboardButton("📦 Bulk Buy"),
    )
...
def admin_panel_keyboard():
    ...
    kb.add(
        ...
        types.KeyboardButton("💰 Set Price/Stock"),
    )
    kb.add(
    ...
    kb.add(
        ...
        types.KeyboardButton("💵 Add/Remove Balance"),
    )
    kb.add(
...
@bot.message_handler(func=lambda m: m.text == "📤 Upload File" and is_admin(m.from_user.id))
def admin_upload_file(message):
    user_state[message.from_user.id] = {"menu": "admin_upload_file"}
    bot.send_message(
        message.chat.id,
        "📤 কোন প্রোডাক্টের জন্য ফাইল আপলোড করছেন সেটার ফাইল পাঠান।\n"
        "(WhatsApp Number / Telegram Number stock ফাইল - .txt/.csv/.xlsx)",
    )
    # TODO: bot.register_next_step_handler(message, process_admin_file_upload)
...
@bot.message_handler(func=lambda m: m.text == "💰 Set Price/Stock" and is_admin(m.from_user.id))
def admin_set_price_stock(message):
    # TODO: প্রোডাক্ট বাছাই -> নতুন price/stock ইনপুট -> products dict/DB আপডেট
    bot.send_message(message.chat.id, "💰 Price/Stock পরিবর্তনের লজিক এখানে যুক্ত হবে।")
...
def admin_users_list(message):
    ...
    bot.send_message(message.chat.id, f"👥 মোট ইউজার: {len(users)}\n(তালিকা লজিক পরে যুক্ত হবে)")
...
def admin_broadcast(message):
    user_state[message.from_user.id] = {"menu": "admin_broadcast"}
    bot.send_message(message.chat.id, "📢 যে মেসেজটি সব ইউজারকে পাঠাতে চান লিখুন।")
    # TODO: bot.register_next_step_handler(message, process_broadcast_message)


@bot.message_handler(func=lambda m: m.text == "💵 Add/Remove Balance" and is_admin(m.from_user.id))
def admin_balance_edit(message):
    # TODO: user_id চাইবে, তারপর amount +/- করবে
    bot.send_message(message.chat.id, "💵 Balance যোগ/বিয়োগ করার লজিক এখানে যুক্ত হবে।")
...
def admin_orders(message):
    ...
    bot.send_message(message.chat.id, f"🧾 মোট অর্ডার: {len(orders)}\n(তালিকা লজিক পরে যুক্ত হবে)")


@bot.message_handler(func=lambda m: m.text == "⚙️ Bot Settings" and is_admin(m.from_user.id))
def admin_bot_settings(message):
