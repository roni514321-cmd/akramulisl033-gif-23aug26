...
async def msg_deposit_trxid(message: Message, state: FSMContext, db_user: User):
    ...
    if method == "Binance":
        ...
        # — তাই স্পেস মুছে ফেলা হয় না, শুধু একাধিক স্পেসকে একটায় normalize
        # করা হয়, ঠিক sms parser যেভাবে normalize করে সেভাবেই।
        trx_id = _normalize_binance_name(raw_input)
        valid = len(trx_id) >= 3 and re.match(r"^[A-Z0-9_. ]+$", trx_id)
    else:
