...
async def init_db():
    ...
    async with SessionLocal() as s:
        defaults = {
            ...
            "withdraw_methods": "bKash,Nagad,Rocket,Bank,Crypto",
...
def kb_withdraw_methods(methods: list[str]) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    for m in methods:
        b.button(text=m, callback_data=WithdrawMethodCB(method=m))
    b.adjust(2)
    return b.as_markup()
...
async def msg_my_sell_requests(message: Message, state: FSMContext):
    ...
    for sub in rows:
        lines.append(
            f"🆔 SUB-{sub.id} | {sub.product_name or '—'} | {sub.item_count} item(s)\n"
...
async def msg_sell(message: Message, state: FSMContext):
    ...
    b.adjust(1)
    await message.answer("💼 <b>Sell Mail</b>\n\nChoose a product to sell:", reply_markup=b.as_markup())
...
async def msg_withdraw_amount(message: Message, state: FSMContext, db_user: User):
    ...
    async with SessionLocal() as s:
        methods_raw = await get_setting(s, "withdraw_methods", "bKash,Nagad,Rocket,Bank,Crypto")
    ...
    await message.answer(
        ...
        reply_markup=kb_withdraw_methods(methods),
    )
...
async def cb_deposit_method(call: CallbackQuery, callback_data: DepMethodCB, state: FSMContext):
    ...
    await safe_edit(
        call,
        f"💳 Method: <b>{callback_data.method}</b>\n\nEnter the deposit amount (number only):{hint}",
        kb_back("wallet"),
    )
    await call.answer()
...
async def cb_sell(call: CallbackQuery):
    ...
    for p in products:
        b.button(text=f"{p.name} — pays {dual_money(p.seller_price)}/item", callback_data=SellProdCB(id=p.id))
    b.adjust(1)
    await safe_edit(call, "💼 <b>Sell Mail</b>\n\nChoose a product to sell:", b.as_markup())
    await call.answer()
...
async def msg_sell_data_xlsx(message: Message, state: FSMContext, db_user: User):
    ...
    kb.adjust(1)
    await broadcast_to_admins(
        f"💼 <b>New Seller Submission (XLSX)</b>\n\n"
        f"👤 User: <code>{db_user.id}</code> (@{db_user.username or '—'})\n"
        f"🆔 ID: SUB-{sub.id}\n"
    ...
    await broadcast_document_to_admins(doc.file_id, caption=f"📎 Raw file — SUB-{sub.id} (from {db_user.id})")
...
async def cb_sell_review(call: CallbackQuery, callback_data: SellReviewCB, state: FSMContext):
    ...
    if callback_data.action == "reject":
        ...
        await safe_edit(call, f"❌ SUB-{sub_id} rejected.", kb_admin_back())
        return
    ...
    await safe_edit(
        call,
        f"✅ <b>SUB-{sub_id}</b> — মোট {sub.item_count} pcs জমা দিয়েছে।\n\n"
...
async def msg_sell_approve_pcs(message: Message, state: FSMContext):
    ...
    try:
        await bot.send_message(
            user.id,
            f"✅ <b>Your submission SUB-{sub.id} was approved!</b>\n\n"
    ...
    await message.answer(
        f"✅ Done. SUB-{sub_id}: {pcs} pcs confirmed → {added} added to stock ({dup} duplicate skipped).\n"
        f"💰 Credited: {dual_money(credit)}",
...
async def cb_admin_deposits(call: CallbackQuery, callback_data: AListCB):
    ...
    if not chunk:
        ...
        for d in chunk:
            lines.append(f"• DEP-{d.id} | user {d.user_id} | {d.method} | {dual_money(d.amount)}")
        text = "\n".join(lines)
    b = InlineKeyboardBuilder()
    for d in chunk:
...
async def cb_admin_sells(call: CallbackQuery, callback_data: AListCB):
    ...
    b = InlineKeyboardBuilder()
    for sub in chunk:
        b.button(text=f"👁 SUB-{sub.id} — {sub.item_count} pcs", callback_data=SellReviewCB(action="view", id=sub.id))
    b.adjust(1)
    nav = []
    if page > 0:
