...
bot_settings = {
    ...
    "method_video_link": "",             # ⚙️ Method বাটনে ইউজারকে দেখানো টিউটোরিয়াল ভিডিও লিংক
}
...
def bot_settings_inline():
    ...
    kb.add(
        types.InlineKeyboardButton(
            "🎥 Method Video Link: " + ("সেট করা আছে ✅" if bot_settings["method_video_link"] else "সেট করা নেই"),
...
def process_deposit_amount(message):
    ...
    if entered is None or entered <= 0:
        bot.send_message(message.chat.id, f"⚠️ সঠিক amount {unit} লিখুন (যেমন: 100)।")
...
def process_deposit_trxid(message):
    ...
    trx_id = text
    if not trx_id:
        bot.send_message(message.chat.id, f"⚠️ সঠিক {_id_label(method)} লিখুন।")
        bot.register_next_step_handler(message, process_deposit_trxid)
        return
    ...
    if method in DEPOSIT_MANUAL_METHODS:
        ...
        bot.send_message(
            ...
            "Admin ভেরিফাই করার পর ব্যালেন্স যুক্ত হবে। পেমেন্ট verify হলে অটো Approve হবে।",
...
def menu_method(message):
    ...
    video_link = bot_settings.get("method_video_link") or ""
    if not video_link:
        bot.send_message(
            message.chat.id,
    ...
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("▶️ Watch Video", url=video_link))
    bot.send_message(
        message.chat.id,
        "⚙️ <b>Method</b>\n\nকিভাবে ডিপোজিট/পারচেজ করবেন তা এই ভিডিওতে দেখুন: 👇",
        reply_markup=kb,
    )
...
def cb_bot_settings(call):
    ...
    if data == "settings_method_video":
        user_state[uid] = {"menu": "admin_settings_method_video"}
        bot.send_message(
            chat_id,
            "🎥 ⚙️ Method বাটনে ইউজারকে দেখানোর জন্য ভিডিও লিংক লিখে পাঠান।\n"
            f"বর্তমান: {bot_settings['method_video_link'] or 'সেট করা নেই'}\n\n"
            "উদাহরণ: https://youtu.be/xxxxxxx",
        )
        bot.register_next_step_handler(call.message, process_settings_method_video)
        return


def process_settings_method_video(message):
    uid = message.from_user.id
    if not is_admin(uid):
        return
    ...
    if not text.startswith(("http://", "https://")):
        ...
        bot.register_next_step_handler(message, process_settings_method_video)
        return

    bot_settings["method_video_link"] = text   # TODO: DB তে persist করুন
    bot.send_message(
        message.chat.id,
        "✅ Method Video Link আপডেট হয়েছে।",
        reply_markup=bot_settings_inline(),
    )
    user_state[uid] = {"menu": "admin_bot_settings"}
...
def process_settings_support_username(message):
    ...
    bot_settings["support_username"] = username   # TODO: DB তে persist করুন
    bot.send_message(
        message.chat.id,
...
def process_balance_edit_amount(message):
    ...
    target["balance"] = new_balance   # TODO: database তে persist করুন (এখনো in-memory)
