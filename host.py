...
def parse_payment_sms(text: str, hint_method: str = "") -> Optional[dict]:
...
async def store_sms_and_try_match(text: str, sender: str = "", hint_method: str = "") -> dict:
    ...
    parsed = parse_payment_sms(text, hint_method)
    if not parsed:
...
async def auto_approve_deposit(session: AsyncSession, dep: Deposit, sms: "SmsMessage") -> bool:
    ...
    try:
        await bot.send_message(
            ...
            f"পেমেন্ট SMS এর সাথে Trx ID ও Amount ম্যাচ হওয়ায় অটোমেটিক অ্যাপ্রুভ হয়ে গেছে। ধন্যবাদ!",
        )
    except Exception:
        pass
    try:
...
async def msg_deposit_amount(message: Message, state: FSMContext):
    ...
    await message.answer(
        "🔑 এখন পেমেন্ট SMS এ পাওয়া <b>Transaction ID (TrxID)</b> টি হুবহু পাঠান।\n"
        "(এই TrxID ও Amount ঠিক থাকলে ডিপোজিট সাথে সাথে অটো-অ্যাপ্রুভ হয়ে যাবে।)"
    )
...
async def msg_deposit_trxid(message: Message, state: FSMContext, db_user: User):
    ...
    await message.answer(
        ...
        f"SMS ম্যাচ এখনো পাওয়া যায়নি — সাথে সাথে অ্যাপ্রুভ না হলে একটু অপেক্ষা করুন অথবা এডমিন ম্যানুয়ালি রিভিউ করবে।",
        reply_markup=kb_back("home"),
    )
    ...
    kb.adjust(2)
    await broadcast_to_admins(
        f"💰 <b>New Deposit Request</b> (SMS ম্যাচ পাওয়া যায়নি)\n\n"
        f"👤 User: <code>{db_user.id}</code> (@{db_user.username or '—'})\n"
        f"💳 Method: {method}\n"
        f"💰 Amount: {dual_money(amount)}\n"
        f"🔑 TrxID: <code>{trx_id}</code>\n"
...
async def sms_webhook_handler(request: web.Request) -> web.Response:
    ...
    # query params (অনেক SMS forwarder অ্যাপ শুধু GET রিকোয়েস্ট পাঠাতে পারে,
    # URL এ ?text={message}&from={from} টেমপ্লেট দিয়ে)
    ...
    if request.method == "POST":
        try:
            ...
            body = {}
        text = text or body.get("text") or body.get("message") or ""
        sender = sender or body.get("from") or body.get("sender") or ""
        hint_method = hint_method or body.get("method") or ""

    if not text:
        return web.json_response({"ok": False, "error": "no_text_field"}, status=400)
