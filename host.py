...
def _otp_link_keyboard(link):
    """OTP link এর জন্য বাটন বানায়। https লিংক হলে Telegram Mini App (WebApp) হিসেবে
    খোলার চেষ্টা করে; http লিংক বা WebApp সাপোর্ট না থাকলে সাধারণ URL বাটনে fallback করে
    (সেটাও Telegram এর ভিতরেই ব্রাউজার খুলে দেখায়, JS ঠিকভাবে রান হয়)।"""
    kb = types.InlineKeyboardMarkup()
    if str(link).startswith("https://"):
        try:
            kb.add(types.InlineKeyboardButton("🔑 OTP দেখুন (Mini App)", web_app=types.WebAppInfo(url=link)))
            return kb
        except Exception:
            pass
