...
def process_deposit_trxid(message):
    ...
    if method in DEPOSIT_MANUAL_METHODS:
        ...
        safe_edit_or_send(
            ...
            "SMS ম্যাচ পেলেই auto-approve হবে। নাহলে Admin ভেরিফাই করার পর ব্যালেন্স যুক্ত হবে।",
            reply_markup=None,
        )
        user_state[uid] = {"menu": "main"}
        admin_text = (
...
def process_settings_method_video_link(message):
    ...
    bot_settings["method_videos"].append({"title": title, "link": text})   # TODO: DB তে persist করুন
    bot.send_message(
        message.chat.id,
